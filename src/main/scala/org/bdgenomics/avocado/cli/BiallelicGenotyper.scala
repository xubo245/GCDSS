...
import org.bdgenomics.avocado.util.{HardFilterGenotypes, HardFilterGenotypesArgs, PrefilterReads, PrefilterReadsArgs}
...
object BiallelicGenotyper extends BDGCommandCompanion {
  ...
  def discoverAndCall(reads: AlignmentRecordRDD,
    ...
    Biallelic.discoverAndCall(reads, ploidy, optDesiredPartitionCount, optPhredThreshold, optMinObservations, optDesiredPartitionSize, optDesiredMaxCoverage)
  }
}
...
class BiallelicGenotyper(
  ...
  def run(sc: SparkContext) {
    ...
    val reads = sc.loadAlignments(args.inputPath, projection = projection)
    if (debug) {
      println("*******************reads count:" + reads.rdd.count())
    }
    // filter reads
    val filteredReads = PrefilterReads(reads, args)
    if (debug) {
      println("*******************filteredReads count:" + filteredReads.rdd.count())
    }
    ...
    if (debug) {
      println("*******************filteredReads.rdd.count:" + filteredReads.rdd.count())
    }
    ...
    val genotypes = Option(args.variantsToCall).fold({
      ...
      if (debug) {
        println("*******************loadVariants:filteredReads.rdd.count:" + filteredReads.rdd.count())
      }
    ...
    if (debug) {
      println("*******************genotypes count:" + genotypes.rdd.count())
    }
    // hard filter the genotypes
    val filteredGenotypes = HardFilterGenotypes(genotypes, args)
    if (debug) {
      println("*******************filteredGenotypes.rdd.count:" + filteredGenotypes.rdd.count())
    }
