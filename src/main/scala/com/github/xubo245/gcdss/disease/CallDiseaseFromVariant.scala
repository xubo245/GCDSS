...
import org.bdgenomics.avocado.cli.DiscoverVariants

/**
  * Created by xubo on 2017/4/9.
  */
object CallDiseaseFromVariant {
  def main(args: Array[String]) {
    val startTime = System.currentTimeMillis()
    var vcfFile = args(0)
    val vcf2omimSimpleFile = args(1)
    var out = args(2)
    var appArgs = "vcfFile:" + vcfFile + "\tout:" + out
    val conf = new SparkConf().setAppName("CallDiseaseFromVariant:" + appArgs)
    ...
    val saveRDD = returnRDD.map { each =>
      ...
      val str = str1(1) + '|' + str1(2) + '|' + str1(3) + '|' + str1(4) + '|' + each._2 + '|' +
        each._3 + '|' + "http://omim.org/entry/"+each._3 + '|' + each._4 + '|' + each._5 + '|' + each._6 + '|' + each._7 + '|' +
